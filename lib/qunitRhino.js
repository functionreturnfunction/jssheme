...
QUnit.Module.prototype.end = function() {
  this.ended = new Date();
};
...
QUnit.ConsolePrinter.prototype.print = function() {
  QUnit.currentModule.end();

  var curModule, curResult, successes, failures, exceptions, moduleTime;
  var totalSuccesses, totalFailures, totalExceptions, totalTests;
  var totalAssertions;
  totalSuccesses = totalFailures = totalExceptions = totalTests =
    totalAssertions = runtime = 0;
  for (var i = 0, leni = this.modules.length; i < leni; ++i) {
    ...
    moduleTime = (curModule.ended - curModule.started);

    print('  ' + lenj + ' tests, ' + successes + ' successful, ' +
          failures + ' failed, ' + exceptions + ' exception' +
          (exceptions == 1 ? '' : 's') + ', ' + curModule.assertions +
          ' assertion' + (curModule.assertions == 1 ? '' : 's') + ', ' +
          moduleTime + 'ms.\n');
    runtime += moduleTime;

    totalAssertions += curModule.assertions;
